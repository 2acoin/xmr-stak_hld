...
enum xmrstak_algo_id
{
	...
	cryptonight_armor = start_derived_algo_id,
	...
	cryptonight_armor = (start_derived_algo_id + 4)
...
inline std::string get_algo_name(xmrstak_algo_id algo_id)
{
	...
	static std::array<std::string, 5> derived_algo_names =
	{{
		"cryptonight_armor",
		"cryptonight_turtle",
...
// cryptonight armor (the mask is not using the full 512kib scratchpad)
constexpr uint32_t CN_ARMOR_MASK = 0x3FFE0;
...
inline xmrstak_algo POW(xmrstak_algo_id algo_id)
{
	...
	static std::array<xmrstak_algo, 5> derived_pow =
	{{
		{cryptonight_armor, cryptonight_monero_v8, CN_ITER/8, CN_MEMORY/8, CN_TURTLE_MASK},
